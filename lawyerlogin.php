...
if (isset($_GET["con"]) && isset($_GET["i"]) && isset($_GET["t"]))
{
	...
	$sql1="Select email from tbllawyer where email = '$email'";
	...
	if ($count > 0){
		...
		$sEmail = $row['email'];
		if(md5($sEmail) == $user){
		$sql="UPDATE tbllawyer set status=$active where email ='$email';";
		mysql_query($sql);
		}else{
...
<body>
	...
	<div id="wrapper">
		...
		<div id="content">
			...
			<form id="login" method="post" action="processing/lawyerLoginPro.php"> 
